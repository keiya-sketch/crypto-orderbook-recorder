...
- **実測で早くも判明**: BTCUSDTのスプレッドは$63,614に対し$0.01(約0.16bp)しかなく、`investment-validation`の`src/experiments/orderbook_cost.py`で計算した理論的執行コストも中央値でtaker手数料(10bps)の0.008%程度=ほぼ無視できる水準だった。ただしp99は中央値の最大300倍・$100,000notionalではtop20階層の22%が depth不足で真のコストを過小評価している疑いがある
- **2026-08-13追加**: 対照として`PEPEUSDT`(スプレッド実測37bps・手数料10bpsを単独で上回る薄い板)を追加。BTCとの比較で「薄い板ならスリッページが手数料を上回るか」を検証する
